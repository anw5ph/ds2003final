...
shinyApp(
  ...
  server = function(input, output){
    pt1 <- reactive({
      ...
      (bbox <- make_bbox(Longitude, Latitude, data = nypd_sex))
      map <- get_stamenmap(bbox, zoom = 12, maptype = "toner-lite")
      ggmap(map, extent = "device") + geom_contour_filled(aes(Longitude, Latitude, z = density), kde_df, alpha = 0.65) + labs(title = "Shooting Density Across the Bronx", subtitle = "NYPD, 2022", x = "lon", y ="lat", caption = "A heat map displaying the distribution of shooting \n incidents across the Bronx in 2022.")
    ...
    pt2 <- reactive({
      ...
      (bbox <- make_bbox(Longitude, Latitude, data = nypd_sex))
      map <- get_stamenmap(bbox, zoom = 12, maptype = "toner-lite")
      ggmap(map, extent = "device") + geom_contour_filled(aes(Longitude, Latitude, z = density), kde_df, alpha = 0.65) + labs(title = "Shooting Density Across Brooklyn", subtitle = "NYPD, 2022", x = "lon", y ="lat", caption = "A heat map displaying the distribution of shooting \n incidents across Brooklyn in 2022.")
...
date_plot = ggplot(nypd2,aes(x=Month))+geom_bar(fill = "royalblue1", color = "royalblue4")+theme_minimal()+labs(y='Number of Incidents', title='Number of Shooting Incidents by Month',subtitle='NYPD, 2022')+theme(plot.title=element_text(hjust=0.5),plot.subtitle=element_text(hjust=0.5),panel.grid.major.x = element_blank())+ylim(0,250)

nypd3=nypd%>%
  separate(OCCUR_TIME, c('Hour','Minute','Second'), ':')
time_plot = ggplot(nypd3,aes(x=Hour))+geom_bar(fill = "royalblue1", color = "royalblue4")+theme_minimal()+labs(y='Number of Incidents',title='Number of Shooting Incidents by Hour',subtitle='NYPD, 2022')+theme(plot.title=element_text(hjust=0.5),plot.subtitle=element_text(hjust=0.5),panel.grid.major.x = element_blank())+ylim(0,150)

sex_plot = ggplot(nypd,aes(x=PERP_SEX))+geom_bar(fill = "royalblue1", color = "royalblue4")+theme_minimal()+labs(x='Sex',y='Number of Perpetrators',title='Number of Perpetrators by Sex',subtitle='NYPD, 2022')+theme(plot.title=element_text(hjust=0.5),plot.subtitle=element_text(hjust=0.5),panel.grid.major.x = element_blank())+ylim(0,1100)

boro_plot = ggplot(nypd,aes(x=BORO))+geom_bar(fill = "royalblue1", color = "royalblue4")+labs(x='Borough',y='Number of Incidents',title='Number of Shooting Incidents by Borough',subtitle='NYPD, 2022')+theme_minimal()+theme(plot.title=element_text(hjust=0.5),plot.subtitle=element_text(hjust=0.5),panel.grid.major.x = element_blank()) + ylim(0,600)

lat_plot = ggplot(nypd,aes(x=Latitude))+geom_histogram(bins=41,fill = "royalblue1", color = "royalblue4")+theme_minimal()+labs(y='Number of Incidents',title='Number of Shooting Incidents by Latitude',subtitle='NYPD, 2022')+theme(plot.title=element_text(hjust=0.5),plot.subtitle=element_text(hjust=0.5),panel.grid.major.x = element_blank())+ylim(0,125)

long_plot = ggplot(nypd,aes(x=Longitude))+geom_histogram(bins=41,fill = "royalblue1", color = "royalblue4")+theme_minimal()+labs(y='Number of Incidents',title='Number of Shooting Incidents by Longitude',subtitle='NYPD, 2022')+theme(panel.grid.major.x = element_blank(), axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))+ylim(0,225)
